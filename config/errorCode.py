def errors(err_code):
    err_dic = {0:('OP_ERR_NONE','정상처리'),
               ...
               -200: ('OP_ERR_SIZE_OVERFLOW','시세조회과부'),
               -201: ('OP_ERR_RQ_STRUCT_FAIL','전문작성초기화실패'),
               -202: ('OP_ERR_RQ_STRING_FAIL','전문작성입력값오류'),
               -203: ('OP_ERR_NO_DATA','데이타없음'),
               -204: ('OP_ERR_OVER_MAX_DATA','조회가능한종목수초과'),
               -205: ('OP_ERR_DATA_RCV_FAIL','데이타수신실패'),
               -206: ('OP_ERR_OVER_MAX_FID','조회가능한FID초과'),
               -207: ('OP_ERR_REAL_CANCEL','실시간해재오류'),
               -300: ('OP_ERR_ORD_WRONG_INPUT','입력값오류'),
               -301: ('OP_ERR_ORD_WRONG_ACCTNO','계좌비밀번호없음'),
               -302: ('OP_ERR_OTHER_ACC_USE','타인계좌사용오류'),
               -303: ('OP_ERR_MIS_5BILL_EXC','주문가격이20억원을초과'),
               -304: ('OP_ERR_MIS_2BILL_EXC','주문가격이50억원을초과'),
